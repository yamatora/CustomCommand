...
# 複数解像度のノイズを重畳
def get_fbm_noise(width, height):
    ...
    for y in range(height):
        for x in range(width):
            ...
            pos = __vec2(x, y)
            res = 0
            res += 0.5 * __get_perlin(pos * 0.5) 
            res += 0.1 * __get_perlin(pos * 1.3)
            res += 0.25 * __get_perlin(pos * 0.9)
            res += 0.15 * __get_perlin(pos)
